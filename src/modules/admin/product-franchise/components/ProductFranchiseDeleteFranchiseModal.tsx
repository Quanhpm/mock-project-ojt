...
interface ProductFranchiseDeleteFranchiseModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function ProductFranchiseDeleteFranchiseModal({
  isOpen,
  onClose,
  onConfirm,
  franchiseId,
  franchiseName,
  isSubmitting = false,
}: ProductFranchiseDeleteFranchiseModalProps) {
      ...
      >
        ...
        <div className="space-y-4 px-6 py-6">
          <p className="text-sm leading-6 text-slate-600">
            This action will remove the current franchise. After deletion, you
            will be redirected back to the Product page.
          </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="grid gap-4 text-sm text-slate-700">
              <div>
                ...
                <p className="mt-2 font-medium">{franchiseName || "--"}</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                  Franchise ID
                </p>
                <p className="mt-2 font-medium">{franchiseId}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 px-6 py-4">
          ...
          >
            ...
            {isSubmitting ? "Deleting..." : "Delete Franchise"}
          </button>
        </div>
      </div>
    </div>
